...
de_c=st.sidebar.radio("Qual escala:")
...
if st.button("Converter",icon='🌡️'):
    if de_c:
        st.write(f'{temp}ºC em Fahrenheit: {conv_c_f(temp)}')
        st.write(f'{temp}ºC em Kelvin: {conv_c_k(temp)}')
    elif de_f:
        st.write(f'{temp}ºf em Celsius: {conv_f_c(temp)}')
        st.write(f'{temp}ºf em Kelvin: {conv_f_k(temp)}')
    elif de_k:
        st.write(f'{temp}K em Celsius: {conv_k_c(temp)}')
        st.write(f'{temp}K em Fahrenheit: {conv_k_f(temp)}')
